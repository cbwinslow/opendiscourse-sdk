...
def congress_members_delegate(context: IngestionContext, update_data: Dict[str, Any]):
    ...
    if update_data["success"]:
        ...
        print(
            f"   Table: {context.table_name} | Members: {processed} | Throughput: {throughput:.1f}/min"
        )
...
def govinfo_documents_delegate(context: IngestionContext, update_data: Dict[str, Any]):
    ...
    if update_data["success"]:
        ...
        print(
            f"   Table: {context.table_name} | Documents: {processed} | Throughput: {throughput:.1f}/min"
        )
...
def setup_all_delegates(monitor=None):
    ...
    if monitor is None:
        ...
        try:
            ...
            db_conn = get_db_connection().__enter__()
            monitor = UniversalProgressMonitor(db_conn)
        except Exception as e:
            print(f"Warning: Could not create progress monitor: {e}")
            return None

    # Congress data
    monitor.register_delegate("congress.gov", "congress.members", congress_members_delegate)
    monitor.register_delegate(
        "congress.gov", "congress.member_terms", congress_member_terms_delegate
    )
