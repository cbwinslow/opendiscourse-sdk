...
class RateLimiter:
    ...
    def _setup_default_limits(self):
        ...
        # OpenStates.org: 30 requests per minute = 0.5 per second (very conservative)
        self.limiters['openstates.org'] = TokenBucket(rate=0.5, capacity=3)
    ...
    def wait(self, api_name: str, tokens: int = 1) -> None:
        ...
        if not limiter.consume(tokens):
            print(f"🐌 Rate limiting active for {api_name} - waiting for tokens...")
...
adaptive_limiters = {
    ...
    'openstates.org': AdaptiveRateLimiter('openstates.org', 0.5, 3),
