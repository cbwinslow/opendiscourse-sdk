...
try:
    ...
    def _save_document(content: str, metadata: DocumentMetadata) -> Optional[int]:
        ...
        try:
            # Parse bill information from metadata
            source_id = metadata.get("source_id", "")
            title = metadata.get("title", "")
            source_url = metadata.get("source_url", "")
            source_date = metadata.get("source_date", "")

            # Parse congress number, bill type, and bill number from source_id
            # Format: "congress-bill_type-bill_number" (e.g., "117-HR-897")
        ...
        try:
            ...
            if text_versions:
                ...
                if not text_url:
                    ...
                    # Fetch bill text
                    # Rate limiting for Congress.gov API (text URLs are also from Congress.gov)
                    rate_limiter.wait("congress.gov")
            ...
            if doc_id:
                ...
                logger.error("Failed to save bill %s-%s to database", bill_type, bill_number)
                self.stats["failed_ingestions"] += 1
                return False

        except Exception as e:
            logger.error("Error processing bill %s-%s: %s", bill_type, bill_number, e)
            self.stats["failed_ingestions"] += 1
            return False
